...
const EMPTY_PERSON = {
  ...
  bio: "", linkedin: "", instagram: "", youtube: "", website: "", is_active: true,
};
...
// PersonFormDialog
// ────────────────────────────────────────────────────────────────────
function PersonFormDialog({ person, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    if (!form.full_name?.trim()) { setErrors({ full_name: "Nome é obrigatório." }); return; }
    setSaving(true);
    try {
      const payload = { ...form };
      if (person?.id) {
        await base44.entities.Person.update(person.id, payload);
      } else {
        await base44.entities.Person.create(payload);
      }
      onSaved();
    } catch (err) {
