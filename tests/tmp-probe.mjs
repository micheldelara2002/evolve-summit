...
for(const p of ['speaker','partner']){
 console.log('\nPERSONA',p);
 for(const [e,query] of [['Participant',{role_in_event:'speaker',is_deleted:false}],['EventMembership',{is_deleted:false}],['PartnerRepresentative',{is_deleted:false}],['Person',{contact_email:p==='speaker'?'contato+speaker@evolveinst.com':'contato+partnermanager@evolveinst.com'}]]){
  const x=await q(p,e,query); console.log(e,JSON.stringify(x));
 }
}
